...
const StorageMode: React.FC<StorageModeProps> = ({ slots, customers }) => {
  ...
  const [approvalModal, setApprovalModal] = useState<{
    ...
    slotId: string;
  }>({ isOpen: false, request: null, slotId: "" });
  const [guidanceModal, setGuidanceModal] = useState(false);
  const [deleteReqModal, setDeleteReqModal] = useState<{
    isOpen: boolean;
    reqId: string | null;
  }>({ isOpen: false, reqId: null });
  ...
  const handleAdjustContract = (slotId: string, start: string, end: string) =>
    adjustStorageContract(slotId, start, end);
  ...
  const getDaysStored = (dateStr?: string) =>
    dateStr
      ...
      : 0;
  const getDaysOnRide = (slot: StorageSlot) =>
    slot.status === "on_ride" && slot.lastActivity
      ? Math.floor(
          (new Date().getTime() - new Date(slot.lastActivity).getTime()) /
            (1000 * 3600 * 24),
        )
      : 0;
  ...
  const getSlotColor = (slot: StorageSlot) => {
    ...
    if (slot.status === "vacant")
      return "bg-slate-100 border-slate-200 text-slate-400 hover:border-blue-400 hover:bg-blue-50 border-dashed";

    if (isOverdue(slot.expiryDate))
      return "bg-red-500 border-red-700 text-white shadow-md animate-pulse";

    if (slot.status === "on_ride")
  ...
  return (
    <div className="flex flex-row h-[calc(100vh-80px)] overflow-hidden bg-slate-50">
            ...
            requests.map((req) => (
              ...
              >
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setDeleteReqModal({ isOpen: true, reqId: req.id });
                  }}
        ...
        <div className="p-6 border-b border-slate-200 bg-white flex justify-between items-center">
          ...
          <div className="flex gap-4 text-xs font-bold uppercase bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 bg-slate-200 rounded-full"></div> Vacant
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 bg-white border border-purple-500 rounded-full"></div>{" "}
              Stored
            </div>
            <div className="flex items-center gap-2">
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start">
                    <span
                      className={`font-mono text-xl font-black ${expired ? "opacity-80 text-white" : "opacity-40"}`}
                    >
                      {slot.id}
                    </span>
                    {slot.status === "occupied" && (
                      <Bike
                        size={18}
                        className={expired ? "text-white" : "text-purple-500"}
                      />
                    )}
                    {slot.status === "on_ride" && (
                      <Clock size={18} className="text-yellow-600" />
                    )}
                    {expired && slot.status === "occupied" && (
                  ...
                  {slot.status === "vacant" ? (
                    <div
                      className={`self-center text-xs font-bold uppercase tracking-widest pointer-events-none transition-opacity ${dragOverSlotId === slot.id ? "opacity-100 text-purple-600" : "opacity-30"}`}
                    >
                    ...
                    <div className="mt-2 flex flex-col h-full justify-end">
                      ...
                      <p
                        className={`font-black text-sm line-clamp-1 leading-tight ${expired ? "text-white" : "text-slate-800"}`}
                      >
                        {slot.customerName}
                      </p>
                      <p
                        className={`text-xs font-bold line-clamp-1 mb-2 ${expired ? "text-red-100" : "opacity-60"}`}
                      >
                        {slot.bikeModel}
                      </p>
                      ...
                      {slot.status === "occupied" && (
                        <div className="space-y-1">
                          <div
                            className={`text-[10px] p-1.5 rounded flex justify-between font-mono ${expired ? "bg-red-700 text-white" : "bg-slate-100 text-slate-600"}`}
                          >
                            <span>In: {daysStored}d</span>
                            <span
                              className={expired ? "font-bold text-white" : ""}
                            >
                              End:{" "}
      ...
      <StorageActionModal
        ...
        onRide={() => {
          if (actionModal.slot) handleRide(actionModal.slot.id);
        }}
        onCheckout={() => {
          if (actionModal.slot) handleCheckout(actionModal.slot.id);
        }}
        onViewHistory={() =>
          setHistoryModal({ isOpen: true, slot: actionModal.slot })
        }
        onExtend={() =>
          setExtendModal({ isOpen: true, slot: actionModal.slot })
        }
      />
      ...
      <StorageReturnModal
        ...
        onConfirm={(photo?: File, notes?: string) =>
          handleReturn(returnModal.slot!.id, photo, notes)
        }
      />

...
const MechanicMode: React.FC<MechanicModeProps> = ({
  ...
  const handleFinish = (ticket: Ticket) =>
    updateTicketStatus(ticket.id, "ready");
  const handleResume = (ticket: Ticket) =>
    updateTicketStatus(ticket.id, "active");
  const handlePending = (ticket: Ticket) =>
    setPendingModal({ isOpen: true, ticket });
  const handleEdit = (ticket: Ticket) => setEditModal({ isOpen: true, ticket });
  const handleChangeMechanic = (ticket: Ticket) =>
    setAssignModal({ isOpen: true, ticket });
    ...
    >
      ...
      >
        <div className="flex items-center gap-3 md:gap-5 w-full sm:w-auto">
          ...
          <div>
            <h1 className="text-xl md:text-3xl font-black text-slate-800 uppercase tracking-tighter italic">
              MODE MEKANIK
            </h1>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="bg-slate-100 px-4 md:px-6 py-2 md:py-3 rounded-xl md:rounded-2xl border-2 border-slate-200 w-full sm:w-auto text-center">
            ...
            <span className="ml-2 text-[10px] font-black text-slate-400 uppercase">
              Unit Sedang Antri
            </span>
          </div>
        </div>
      ...
      <div
        className={`grid gap-4 md:gap-6 ${isFullScreen ? "grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" : "grid-cols-1 lg:grid-cols-2"}`}
      >
        {ongoingTickets.length === 0 ? (
          <div className="col-span-full text-center py-20 md:py-32 bg-white rounded-[2rem] border-4 border-dashed border-slate-200">
            ...
            <h3 className="text-xl md:text-2xl font-black text-slate-300 uppercase italic">
              Bengkel Tenang
            </h3>
          ...
          ongoingTickets.map((ticket) => {
            const displayId = ticket.ticketNumber
              ? `#${ticket.ticketNumber}`
              : `#${ticket.id.slice(-4)}`;
              ...
              >
                ...
                <div className="flex flex-col gap-1 pr-12 md:pr-0">
                  <div className="flex items-center gap-2 md:gap-3">
                    <span className="font-mono text-2xl md:text-4xl font-black text-slate-300">
                      {displayId}
                    </span>
                    <h3 className="text-xl md:text-3xl font-black text-slate-800 line-clamp-1 break-words">
                      {ticket.customerName}
                    </h3>
                  </div>
                  <p className="text-sm md:text-xl font-extrabold text-blue-600 italic uppercase tracking-tighter break-words">
                    {ticket.unitSepeda}
                  </p>
                </div>
                ...
                <div className="bg-slate-50 p-3 md:p-5 rounded-xl md:rounded-2xl border-2 border-slate-100">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {ticket.serviceTypes.map((svc, i) => (
                      <span
                        key={i}
                        className="bg-white border-2 border-slate-200 text-slate-800 text-[10px] md:text-sm font-black px-2 md:px-4 py-1 rounded-lg md:rounded-xl shadow-sm"
                      >
                        {svc}
                      </span>
                    ))}
                  ...
                  {ticket.mechanic && (
                    <div className="mt-3 pt-2 border-t border-slate-200 flex justify-between items-center">
                      <p className="text-[10px] md:text-xs font-black text-slate-500">
                        Mekanik:{" "}
                        <span className="text-blue-700">{ticket.mechanic}</span>
                      </p>
                      <button
      ...
      <PendingModal
        ...
        onConfirm={(id: string, reason: string) =>
          updateTicketStatus(id, "pending", undefined, reason)
        }
      />
      ...
      <AssignMechanicModal
        ...
        mechanics={mechanics}
        onAssign={(id: string, mechanic: string) =>
          updateTicketStatus(id, "active", mechanic)
        }
      />
    </div>
  );
